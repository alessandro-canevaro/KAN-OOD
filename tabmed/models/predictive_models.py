...
class ResNet(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        for block in self.blocks:
            self.add_module(str(id(block)), block)
    ...
    def forward(self, x: Tensor, return_features_list=False) -> Tensor:
        x = self.first_layer(x)
        features_list = [x.detach().clone()]
        for block in self.blocks:
            x = block(x)
            features_list.append(x.detach().clone())
        #x = self.blocks(x)
        x_linear, x_features = self.head(x)
        features_list.append(x_features.detach().clone())
        if return_features_list:
            return x_linear, x_features, features_list
        return x_linear, x_features
...
class Transformer(nn.Module):
    ...
    def forward(self, x: Tensor, return_features_list=False) -> Tensor:
        ...
        for layer_idx, layer in enumerate(self.blocks):
            ...
            x = layer['output'](x)
            feature_list.append(torch.mean(x.detach().clone(), dim=1))

        x_linear, x_features = self.head(x)
        feature_list.append(x_features.detach().clone())
        if return_features_list:
            return x_linear, x_features, feature_list
        else:
            return x_linear, x_features
...
class FTTransformer(nn.Module):
    ...
    def forward(self, x_num: Optional[Tensor], x_cat: Optional[Tensor], return_features_list=False) -> Tensor:
        x = self.feature_tokenizer(x_num, x_cat)
        x = self.cls_token(x)
        if return_features_list:
            x_linear, x_features, features_list = self.transformer(x, return_features_list=True)
            features_list.insert(0, x_num.detach().clone())
            return x_linear, x_features, features_list
        else:
...
def apply_model(model, x_num, x_cat=None, return_features=False, return_features_list=False):
    if isinstance(model, FTTransformer):
        if return_features_list:
            out_linear, out_features, out_features_list = model(x_num, x_cat, return_features_list=True)
            return out_linear, out_features_list
        ...
        if return_features_list:
            out_linear, out_features, out_features_list = model(x_num, return_features_list=True)
            return out_linear, out_features_list
