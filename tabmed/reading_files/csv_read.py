...
def read(files_path='', mimic_version='iv'):
    ...
    data_path = os.getenv('OOD_DATASETS_PATH')

    if data_path:
        eICU_path = os.path.join(data_path, 'ood', 'eICU_preprocessed.csv')
    else:
        eICU_path = 'data/eICU_preprocessed.csv'

    eICU_features = pd.read_csv(eICU_path).drop(columns=['Unnamed: 0'])
